...
% NOTE: NOISE CAN BE ADDED TO THE DRIVE, CF QUESTION 7.
...
w_limb_axis = 0;                   % from limb to axial oscillators
...
% MODIFY THE VALUES OF THE SATURATION FUNCTIONS (E.G. TO MODIFY INTRINSIC
% FREQUENCIES, SEE QUESTION 7.C
...
% noise sigma
starttime = ceil(length(times)/4);
sigma = linspace(0,0.9,length(times)-starttime+1);

% Euler integration, starting with second time step as the first time corresponds to initial conditions
for i = 2:length(times)
    ...
    %if i > starttime
    %    theta(i - 1, 4) = normrnd(theta(i-1,4), sigma(i+1-starttime));
    %end
    %if i == round(length(times)/2)
    %    theta(i-1,:) = -theta0;
    %end
